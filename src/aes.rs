...
fn sub_bytes(block: &[u8; 16]) -> [u8; 16] {
...
fn key_schedule(key: &[u8; 16]) -> [[u8; 16]; 11] {
    ...
    for round in 1..11 {
        let round_fn = |key: &[u8]| -> [u8; 4] {
            [
                S_BOX[key[1] as usize] ^ masks[round],
                S_BOX[key[2] as usize],
                S_BOX[key[3] as usize],
                S_BOX[key[0] as usize],
            ]
        };
...
impl AES {
    ...
    pub fn encrypt_block(&self, block: &[u8; 16]) -> [u8; 16] {
        ...
        let mut state = xor_blocks(block, &self.round_keys[0]);

        for i in 1..10 {
            state = mix_columns(&shift_rows(&sub_bytes(&state)));
            state = xor_blocks(&state, &self.round_keys[i]);
        }
        state = shift_rows(&sub_bytes(&state));
        xor_blocks(&state, &self.round_keys[10])
    }
}
